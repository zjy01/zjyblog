...
$diary=M("diary,user");
$diaryAll=$diary->select("count(diary_id) as count")->where("diary.user_id=user.user_id and user.user_id='{$_SESSION['user_id']}'")->sql();
//总数
$allNum=$diaryAll[0]['count'];

//分页查询
$diaryArr=$diary->select()->where("diary.user_id=user.user_id and user.user_id='{$_SESSION['user_id']}'")->orderby('diary_time','desc')->limit($page*$num,$num)->sql();
